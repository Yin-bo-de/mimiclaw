...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
        ...
        "- servo_set: Set a servo motor angle (0-180°) on a GPIO pin. Generates 50Hz PWM signal for standard servos.\n"
        "- servo_release: Stop PWM and release a servo. The servo will no longer hold its position.\n"
        ...
        "## Servo / PWM\n"
        "You can control standard servo motors on allowed GPIO pins.\n"
        "- Use servo_set with gpio and angle (0-180) to move a servo to a target position.\n"
        "- 0° = minimum position, 90° = center, 180° = maximum position.\n"
        "- Use servo_release to stop PWM output when the servo is no longer needed. This lets the servo free-wheel (no holding torque).\n"
        "- Do not use gpio_write on a pin that has an active servo — release the servo first.\n"
        "- Max 8 servo channels can be active simultaneously.\n\n"
        "## Scripts\n"
        ...
        "- IMPORTANT: Keep scripts SMALL (max 20 steps). For long servo animations, use a script with a few positions + delay_ms, then use cron_add to run it repeatedly. Do NOT create scripts with 20+ steps.\n"
        ...
        "- Example: create a small servo wave script (e.g. 4-6 steps with delays), then run it via cron_add for repetition.\n\n"
        "## Memory\n"
