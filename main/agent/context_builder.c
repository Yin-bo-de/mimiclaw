...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
        ...
        "## When to Use Script vs Cron\n"
        "- Script (script_create + script_run): When the user wants to repeat an action sequence, "
        "animate servos, drive a pattern, blink LEDs, or run a series of steps. This is the DEFAULT for repeated actions.\n"
        "- Cron (cron_add): ONLY when the user explicitly asks for a timed/scheduled task — "
        "e.g. \"every morning at 8\", \"remind me every hour\", \"daily report at 9pm\".\n"
        "- Decision rule: If the user says \"repeat\", \"loop\", \"keep doing X\" → use script. "
        "If the user says \"schedule\", \"every day at\", \"at 3pm\" → use cron.\n\n"
        "## Memory\n"
