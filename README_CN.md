...
- **墨水屏控制** — 实时查看和控制墨水屏显示内容
  - 自动拉取当前状态（天气、待办、金句）
  - 编辑城市与天气摘要，动态增删待办（最多 5 项）
  - 修改每日金句，手动触发屏幕重绘
- **Agent 网页聊天室** — 直接在浏览器中与 AI Agent 对话。消息通过 WebSocket 推入 Agent 循环，AI 回复以气泡形式实时显示，最多保留 100 条消息，自动滚动。
- **文件管理器** — 浏览、读取、编辑和删除设备 SPIFFS 文件系统中的文本文件。可快速修改 `SOUL.md`、`USER.md`、`MEMORY.md` 等配置文件，无需重新烧录。
- **拍屏图片上传** — 从手机或电脑上传任意图片。浏览器自动将图片缩放至 128×296，通过 Floyd-Steinberg 抖动算法映射到墨水屏 4 色（黑/白/黄/红），打包为原生 2bpp 帧缓冲格式后发送到屏幕显示。
- **GPIO 遥控面板** — 以网格形式查看和切换 GPIO 引脚电平。仅允许白名单中的引脚操作，系统保留引脚（如墨水屏相关引脚）已被安全屏蔽。

前端为零依赖的 Vanilla JS 单页应用，通过 WebSocket 与固件实时通信，断线后自动重连。
